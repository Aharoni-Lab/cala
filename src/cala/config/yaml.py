...
from cala.models.config import AbsoluteIdentifier, ConfigID, ConfigSource, valid_config_id
...
class ConfigYAMLMixin(BaseModel, YAMLMixin):
     ...
     * `cala_model` - fully-qualified module path to model class
     * `cala_version` - version of cala when this model was created
    ...
    def from_id(cls: type[Self], id: ConfigID) -> Self:
        ...
        for config_file in chain(*globs):
            try:
                file_id = yaml_peek("cala_id", config_file)
            except KeyError:
                continue
    ...
    @field_validator("cala_model", mode="before")
    @classmethod
    def fill_cala_model(cls, v: str | None) -> AbsoluteIdentifier:
    ...
    def _yaml_header(cls, instance: Self | dict) -> dict:
        if isinstance(instance, dict):
            model_id = instance.get("cala_id", None)
            cala_model = instance.get("cala_model", cls._model_name())
            cala_version = instance.get("cala_version", version("cala"))
        else:
            model_id = getattr(instance, "cala_id", None)
            cala_model = getattr(instance, "cala_model", cls._model_name())
            cala_version = getattr(instance, "cala_version", version("cala"))
        ...
        if model_id is None:
            ...
            model_id = getattr(cls.model_fields.get("cala_id", None), "default", None)
            if type(model_id).__name__ == "PydanticUndefinedType":
                model_id = None

        return {
            "cala_id": model_id,
            "cala_model": cala_model,
            "cala_version": cala_version,
        }
