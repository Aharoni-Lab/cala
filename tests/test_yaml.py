...
from cala.config.yaml import ConfigYAMLMixin, YAMLMixin, yaml_peek
...
a: 9
cala_id: "my-config"
cala_model: "tests.test_yaml.MyModel"
cala_version: "{version("cala")}"
b: "10\"""",
            id="not-at-start",
        ),
        pytest.param(
...
def test_complete_header(tmp_config_source, src: str):
    ...
    assert loaded["cala_version"] == version("cala")
    assert loaded["cala_id"] == "my-config"
    assert loaded["cala_model"] == MyModel._model_name()

    # the header should come at the top!
    lines = loaded_str.splitlines()
    for i, key in enumerate(("cala_id", "cala_model", "cala_version")):
