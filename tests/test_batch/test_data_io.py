...
from cala.batch.data_io import DataIO, VideoMetadata
...
def test_dataio_metadata():
    ...
    with patch("cala.batch.data_io.Image") as MockImage:
...
def test_dataio_save_data(tmp_path):
    ...
    with patch("cala.batch.data_io.Image") as MockImage, patch(
        "cala.batch.data_io.av.open"
    ) as mock_av_open, patch("cala.batch.data_io.tqdm", lambda x, *args, **kwargs: x):
