...
from cala.streaming.init.odl.overlap_groups import (
    OverlapGroupsInitializer,
    OverlapGroupsInitializerParams,
)
from cala.streaming.stores.odl import OverlapGroups


class TestOverlapGroupsInitializerParams:
    """Test suite for OverlapGroupsInitializerParams."""

    @pytest.fixture
    def default_params(self):
        """Create default parameters instance."""
        return OverlapGroupsInitializerParams()
    ...
    def test_validation_valid_spatial_axes(self):
        ...
        params = OverlapGroupsInitializerParams(spatial_axes=("y", "x"))
    ...
    def test_validation_invalid_spatial_axes(self):
        ...
        with pytest.raises(ValueError):
            params = OverlapGroupsInitializerParams(spatial_axes=("height",))
            params.validate()


class TestOverlapGroupsInitializer:
    """Test suite for OverlapGroupsInitializer."""
    ...
    def initializer(self):
        """Create OverlapGroupsInitializer instance."""
        return OverlapGroupsInitializer(OverlapGroupsInitializerParams())

    def test_initialization(self, initializer):
        """Test proper initialization."""
        assert isinstance(initializer.params, OverlapGroupsInitializerParams)
        assert not hasattr(initializer, "overlap_groups_")

    def test_learn_one(self, initializer, sample_footprints):
        """Test learn_one method."""
        initializer.learn_one(sample_footprints)

        # Check that overlap_groups_ was created
        assert hasattr(initializer, "overlap_groups_")
        assert isinstance(initializer.overlap_groups_, xr.DataArray)

        # Check dimensions
        assert initializer.overlap_groups_.dims == ("components", "components")
        assert initializer.overlap_groups_.shape == (5, 5)

        # Check coordinates
        assert "id_" in initializer.overlap_groups_.coords
        assert "type_" in initializer.overlap_groups_.coords
    ...
    def test_transform_one(self, initializer, sample_footprints):
        ...
        # Check result type
        assert isinstance(result, OverlapGroups)
